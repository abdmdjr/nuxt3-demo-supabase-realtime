...
const onUpdate = () => {
  coins.value.forEach(async (coin) => {
    ...
    await supabase.from("coins").update([{ id: coin.id, price: newPrice }]);
  });
};
  ...
  <main className="bg-gray-800 min-h-screen flex flex-col items-center justify-center py-20">
    <div class="flex flex-col items-center space-y-4 mb-10 px-4">
      <button @click="onUpdate"
        ...
        class="mt-3 w-full inline-flex items-center justify-center px-4 py-2 border border-transparent shadow-sm font-medium rounded-md text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500">
